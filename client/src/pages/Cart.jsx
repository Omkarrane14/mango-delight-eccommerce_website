  ...
  const handlePaymentSuccess = async (paymentIntentId) => {
    try {
      ...
      const response = await axios.post(
        "https://mangoshop.onrender.com/api/orders/place-order",
        {
          cartItems,
          totalAmount,
          address,
      ...
      if (response.data.success) {
        const orderId = response.data.orderId; // Ensure orderId is returned from API
        ...
        for (const item of cartItems) {
          ...
          try {
            await axios.delete("https://mangoshop.onrender.com/api/users/cart/remove", {
              headers: {
                Authorization: `Bearer ${authToken}`,
              },
              data: { productId },
        ...
        setTimeout(() => {
          window.location.href = `/orderbill/${orderId}`; // Navigate to order bill page
        }, 1500);
      } else {
...
const Cart = () => {
  ...
  const fetchCartItems = async () => {
    ...
    try {
      const response = await axios.get(
        "https://mangoshop.onrender.com/api/users/cart/",
        {
          headers: { Authorization: `Bearer ${authToken}` },
        }
      );
  ...
  const updateQuantity = async (productId, newQuantity) => {
    try {
      await axios.post(
        `https://mangoshop.onrender.com/api/users/cart/update`,
  ...
  const handleSubmitAddress = async () => {
    ...
    try {
      const { data } = await axios.post(
        "https://mangoshop.onrender.com/api/orders/create-payment-intent",
        {
          amount: totalPrice,
          customerAddress: {
                ...
                {cartItems.map((item) => (
                  <tr key={item.product._id}>
                    <td className="d-flex align-items-center">
                      <img
                        src={`https://mangoshop.onrender.com/uploads/${item.product.images}`}
